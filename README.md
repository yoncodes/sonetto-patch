...
- Reads endpoints from `sonetto.toml`, so no system proxy or hard-coded private server address is required. An optional exact process-local proxy endpoint can be configured for CONNECT fallback without intercepting unrelated loopback traffic.
...
- 从 `sonetto.toml` 读取服务端地址，不需要修改系统代理，也不在源码中硬编码私服地址；可选配置精确的进程内代理端点作为 CONNECT 兜底，不会拦截其他回环流量。
